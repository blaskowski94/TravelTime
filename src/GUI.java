...
public class GUI extends Application {
	...
	private LineChart[] lineChart = new LineChart[7];
	private volatile boolean					running		= false;
	private static Processor					process		= new Processor();
	private XYChart.Series[] series1 = new XYChart.Series[7];
	private XYChart.Series[] series2 = new XYChart.Series[7];
	...
	static String getStartAddress() {
		return startTF.getText();
	}

	static String getEndAddress() {
	...
	public void start(Stage stage) throws Exception {
		...
		close.setOnAction(t -> Platform.exit());
		...
		setTimeInterval.setOnAction(t -> timeIntStage.showAndWait());
...
//		});
		...
		imgExport.setOnAction(t -> imgExportStage.showAndWait());
	...
	private void setTimeInterval(String seconds) {
		if (!seconds.equals("")) {
			int timeInt;
			try {
	 ...
	 */
	private void makeChart(DayOfWeek dow) throws Exception {
	...
	private void updateChart() throws Exception {
		while (!process.h2wQ.isEmpty()) {
			...
			Platform.runLater(() -> series1[temp.getDayOfWeekEnum().ordinal()].getData()
		...
		while (!process.w2hQ.isEmpty()) {
			...
			Platform.runLater(() -> series2[temp.getDayOfWeekEnum().ordinal()].getData()
					.add(new XYChart.Data<String, Number>(temp.getTime(), temp.getTimeData().getTotalMins())));
		}
	}
	 ...
	 */
	private void saveAsPng(LineChart<String, Number> lineChart) {
		LocalDateTime ldt = LocalDateTime.now();
		DateTimeFormatter dtf = DateTimeFormatter.ISO_DATE;
		String date = ldt.format(dtf);
		FileChooser fileChooser = new FileChooser();
